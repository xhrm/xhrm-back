...
EOF
        ...
        cd /usr/src
        wget https://api.github.com/repos/p4gefau1t/trojan-go/releases >/dev/null 2>&1
        latest_version=`grep tag_name latest| awk -F '[:,"v]' '{print $6}'`
        rm -f latest
        green "开始下载最新版trojan amd64"
        wget https://github.com/p4gefau1t/trojan-go/v${latest_version}/trojan-go${latest_version}-linux-amd64.tar.xz
         ...
         cat > /usr/src/trojan/server.conf <<-EOF
{
  ...
  "remote_port": "80",
  "log_level": 1,
  "log_file": "/usr/src/trojan-go-access.log",
...
start_menu(){
    clear
    green " ======================================="
    green " 介绍: Trojan go xhrm 20220211      "
