...
EOF
    ...
    if [ ! -d "/usr/src/trojan-go-cert" ]; then
        mkdir /usr/src/trojan-go-cert /usr/src/trojan-go-temp
        mkdir /usr/src/trojan-go-cert/$your_domain
        if [ ! -d "/usr/src/trojan-go-cert/$your_domain" ]; then
            red "不存在/usr/src/trojan-go-cert/$your_domain目录"
            exit 1
        fi
    ...
    elif [ -f "/usr/src/trojan-go-cert/$your_domain/fullchain.cer" ]; then
        cd /usr/src/trojan-go-cert/$your_domain
...
EOF
        ...
        #trojan-go_passwd=$(cat /dev/urandom | head -1 | md5sum | head -c 8)
         rm -rf /usr/src/trojan-go/server.conf
         cat > /usr/src/trojan-go/server.conf <<-EOF
{
  "run_type": "server",
  "local_addr": "0.0.0.0",
  "local_port": 443,
  ...
  "ssl": {
    ...
    "cert": "/usr/src/trojan-go-cert/$your_domain/fullchain.cer",
    "key": "/usr/src/trojan-go-cert/$your_domain/private.key",
    "key_password": "",
    "cipher": "",
    "curves": "",
...
EOF
        cd /usr/src/trojan-go-cli/
        rm -rf /usr/src/trojan-go-temp/
        rm -f /usr/src/trojan-go-cli.zip
        trojan-go_path=$(cat /dev/urandom | head -1 | md5sum | head -c 16)
        #mkdir /usr/share/nginx/html/${trojan-go_path}
        #mv /usr/src/trojan-go-cli/trojan-go-cli.zip /usr/share/nginx/html/${trojan-go_path}/	
...
ExecStart=/usr/src/trojan-go/trojan-go -c "/usr/src/trojan-go/server.conf"  
...
EOF
        ...
        ~/.acme.sh/acme.sh  --installcert  -d  $your_domain   \
            --key-file   /usr/src/trojan-go-cert/$your_domain/private.key \
            --fullchain-file  /usr/src/trojan-go-cert/$your_domain/fullchain.cer \
...
function repair_cert(){
    ...
    if [ $real_addr == $local_addr ] ; then
        ...
        ~/.acme.sh/acme.sh  --installcert  -d  $your_domain   \
            ...
            --fullchain-file /usr/src/trojan-go-cert/$your_domain/fullchain.cer \
            --reloadcmd  "systemctl restart trojan-go"
        if test -s /usr/src/trojan-go-cert/$your_domain/fullchain.cer; then
            green "证书申请成功"
...
function update_trojan-go(){
    /usr/src/trojan-go/trojan-go -v 2>trojan-go.tmp
    curr_version=`cat trojan-go.tmp | grep "trojan-go" | awk '{print $4}'`
    wget  https://api.github.com/repos/p4gefau1t/trojan-go/releases/latest >/dev/null 2>&1
    latest_version=`grep tag_name latest| awk -F '[:,"v]' '{print $6}'`
    rm -f latest
    rm -f trojan-go.tmp
    if version_lt "$curr_version" "$latest_version"; then
        ...
        systemctl restart trojan-go
    /usr/src/trojan-go/trojan-go -v 2>trojan-go.tmp
    green "服务端trojan-go升级完成，当前版本：`cat trojan-go.tmp | grep "trojan-go" | awk '{print $4}'`，客户端请在trojan-go github下载最新版"
    rm -f trojan-go.tmp
    else
