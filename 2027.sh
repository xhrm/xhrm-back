...
SYSTEM_CRON_FILE="/etc/cron.d/system-autoreboot"
DEFAULT_SYSTEM_REBOOT_TIME="05:00"
...
setSystemAutoReboot() {
    HOUR=${DEFAULT_SYSTEM_REBOOT_TIME%:*}
    MINUTE=${DEFAULT_SYSTEM_REBOOT_TIME#*:}
    echo "$MINUTE $HOUR * * * root /sbin/reboot" > $SYSTEM_CRON_FILE
    chmod 644 $SYSTEM_CRON_FILE
}

modifySystemAutoReboot() {
    read -p "请输入新的系统重启时间(HH:MM)：" NEW_TIME
    [[ ! $NEW_TIME =~ ^([01]?[0-9]|2[0-3]):[0-5][0-9]$ ]] && return 1
    HOUR=${NEW_TIME%:*}
    MINUTE=${NEW_TIME#*:}
    echo "$MINUTE $HOUR * * * root /sbin/reboot" > $SYSTEM_CRON_FILE
    chmod 644 $SYSTEM_CRON_FILE
    echo "系统自动重启时间已更新为 $NEW_TIME"
}
...
menu() {
    ...
    echo -e "  ${GREEN}9.${PLAIN} 修改go定时重启时间"
    echo -e "  ${GREEN}10.${PLAIN} 修改系统定时重启"
    echo " -------------"
